  ...
  return (
    <nav className='flex items-center'>
      <div className='flex items-center'>
        <div className='text-20 font-bold mr-2'> Credit Card </div>
        {isDarkMode ? (
          <BsSunFill size={'24px'} color='#e9c46a' className='cursor-pointer' onClick={() => toogleDarkMode()} />
        ) : (
          <FaMoon size={'24px'} color='#e9c46a' className='cursor-pointer' onClick={() => toogleDarkMode()} />
        )}
      </div>
      <ul className='md:flex md:gap-10 ml-auto text-16 font-semibold'>
        ...
        {openMenu && (
          <div className='absolute right-8 bg-white z-10 rounded-lg p-8 text-center text-black text-13'>
